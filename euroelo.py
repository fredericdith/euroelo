...
top_10 = latest_before_date.sort_values(by='EloRating', ascending=False).head(10)
top_10 = top_10[['Team', 'DaysSinceLastGame', 'EloRating']].reset_index(drop=True)
top_10.index += 1
top_10.rename_axis('Rank', inplace=True)
top_10.reset_index(inplace=True)
st.dataframe(top_10, use_container_width=False, hide_index=True)

# --- Elo chart with date picker ---
st.markdown("---")
...
if selected_teams:
    # Compute current rankings once by taking latest EloRating per team
    latest_ratings = (
    ...
    for team in selected_teams:
        ...
        # Get current rating and rank for the team
        team_rating = latest_ratings.loc[latest_ratings['Team'] == team, 'EloRating'].values[0]
        team_rank = latest_ratings.loc[latest_ratings['Team'] == team, 'Rank'].values[0]

        # Show metrics side by side
        col1, col2 = st.columns(2)
        col1.metric("Current Rank", f"#{team_rank}")
        col2.metric("Current Elo Rating", f"{team_rating:.0f}")

        team_data = team_elo[team_elo['Team'] == team].sort_values(by='Date', ascending=False).head(10)
        #display_data = team_data[['Date', 'Opponent', 'HomeAway', 'EloRating', 'Multiplier', 'Delta']]
        display_data = team_data[['Date', 'Opponent', 'HomeAway', 'EloRating', 'Delta']]
        st.dataframe(display_data.reset_index(drop=True))
else:
    st.write("Select a team to see their last 10 Elo games.")


# --- Narrative presets ---
st.markdown("---")
st.header("Narrative presets (⚠️ EXPERIMENTAL ⚠️)")
...
narratives = {
    "Xabi Alonso's historical run with Leverkusen ⬛️🟥": {
        ...
        "description": "Leverkusen went undefeated and won their first-ever Bundesliga title in 2023–24, under Xabi Alonso. This season marks a major historical breakthrough (+167.079).",
    },    
    "Pep Guardiola's rise to the top 🔝": {
        ...
        "description": "On 1 February 2016, Manchester City signed Guardiola for the start of the 2016–17 season. Guardiola guided City to their first Champions League final in 2020–21, and their first Champions League title as part of his second continental treble in 2022–23 (+230.602).",
        "markers": [
    ...
    "Hansi Flick's 2020 Bayern is the GOAT 🐐": {
        ...
        "description": "Bayern's 2020 team under Hansi Flick (2019-11-03 to 2021-06-30) dominated every opponent, winning the Champions League with a perfect record and crushing top teams (like Barcelona 8–2), leading to a record high EuroElo of 2,024.048 in November 2020.",
    "markers": [
    ...
    "The 1950+ elite tier 🔝": {
        ...
        "end_date": datetime.date.today(),
        "description": "Only 5 clubs have crossed the 1950 EuroElo rating in the past 10 years - and it happened just 9 times. This can be seen as the marker of the elite-defining and dominating teams.",
    "markers": [
